...
import { createTestClient, assertSuccessResponse, assertHasLogs, assertLatencyInRange } from '../test-utils';
...
describe('Transport Modes Integration Tests', () => {
  ...
  describe('SIMULATE Mode', () => {
    it('should simulate transaction without broadcasting', async () => {
    ...
    it('should have minimal latency (local simulation)', async () => {
    ...
    it('should use simulate() helper method', async () => {
  ...
  describe('VANILLA Mode', () => {
    it('should execute via standard RPC', async () => {
      const response = await createTransferBuilder()
    ...
    it('should have reasonable latency (standard RPC)', async () => {
      const start = Date.now();
      const response = await createTransferBuilder()
        .setTransport('VANILLA')
        .send();
  ...
  describe('NOZOMI Mode', () => {
    it('should execute via Temporal Nozomi endpoint', async () => {
      const response = await createTransferBuilder()
        .setTransport('NOZOMI')
        .send();
  ...
  describe('ZERO_SLOT Mode', () => {
    it('should execute via 0Slot endpoint', async () => {
      const response = await createTransferBuilder()
    ...
    it('should have ultra-low latency', async () => {
      const start = Date.now();
      const response = await createTransferBuilder()
        .setTransport('ZERO_SLOT')
        .send();
  ...
  describe('HELIUS_SENDER Mode', () => {
    it('should execute via Helius sender service', async () => {
      const response = await createTransferBuilder()
        .setTransport('HELIUS_SENDER')
        .send();
  ...
  describe('JITO Mode', () => {
    it('should execute via Jito with MEV protection', async () => {
      const response = await createTransferBuilder()
        .setTransport('JITO')
    ...
    it('should have acceptable latency with bribe', async () => {
      const start = Date.now();
      const response = await createTransferBuilder()
        .setTransport('JITO')
        .setBribe(2_000_000)
  ...
  describe('NONCE Mode (Multi-Broadcast)', () => {
    it('should execute via multi-broadcast strategy', async () => {
      const response = await createTransferBuilder()
        .setTransport('NONCE')
        .send();
    ...
    it('should have fastest latency (parallel broadcast)', async () => {
      const start = Date.now();
      const response = await createTransferBuilder()
        .setTransport('NONCE')
        .send();
    ...
    it('should be default transport mode', async () => {
  ...
  describe('Transport Comparison', () => {
    it('should execute same operation across all modes', async () => {
      const modes: Array<'SIMULATE' | 'VANILLA' | 'NOZOMI' | 'ZERO_SLOT' | 'HELIUS_SENDER' | 'JITO' | 'NONCE'> = [
        'SIMULATE',
        'VANILLA',
        'NOZOMI',
        'ZERO_SLOT',
    ...
    }, TEST_TIMEOUTS.INTEGRATION * 2); // Allow extra time for all modes
  });

  describe('Transport Mode Switching', () => {
    it('should allow changing transport mode between requests', async () => {
  ...
  describe('Response Consistency', () => {
    it('should return consistent response structure across modes', async () => {
